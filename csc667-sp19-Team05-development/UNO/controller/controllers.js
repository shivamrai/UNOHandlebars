...
UnoController.login = function(request,response){
  ...
  // if(request.session.username !== 'No Session Set'){
...
UnoController.creategame = function(request,response){
  ...
  var sql = "INSERT INTO ROOM (room_name, player_1) VALUES ('" + room + "' , '" + request.session.username + "');"
...
UnoController.getRooms = function(request,response){
    ...
    console.log(result);
    const responseObj = { x: result };
    if(request.session && request.session.username){
      responseObj['username'] = request.session.username 
    }
    response.render('lobby', responseObj);  
	});
}
...
UnoController.renderGame = function(room_name, request, response){
  ...
  response.render('game', { results: result[0] , username: request.session.username});  
  });
}
...
UnoController.joinGame = function(request,response){
  ...
  db.query(sql, function (err, result) {
    ...
    //Player is not on the list so add them into the next available Slot
      ...
      if ((result[0].player_1 != null & result[0].player_2 != null) & (result[0].player_3 != null & result[0].player_4 != null)){
        ...
        var sql2 = "UPDATE ROOM SET player_1 = '" + request.session.username + "' WHERE room_name = '" + request.body.room_name + "'";
        db.query(sql2, function (err, result) {
        if (err) throw err;
        });

      } else if (result[0].player_2 == null){
        var sql3 = "UPDATE ROOM SET player_2 = '" + request.session.username + "' WHERE room_name = '" + request.body.room_name + "'";
        db.query(sql3, function (err, result) {
        if (err) throw err;
        });
      } else if (result[0].player_3 == null){
        var sql4 = "UPDATE ROOM SET player_3 = '" + request.session.username + "' WHERE room_name = '" + request.body.room_name + "'";
        db.query(sql4, function (err, result) {
        if (err) throw err;
        });
      } else if (result[0].player_4 == null){
        var sql5 = "UPDATE ROOM SET player_4 = '" + request.session.username + "' WHERE room_name = '" + request.body.room_name + "'";
  ...
  response.redirect('/game/' + request.body.room_name);
}

/**
 * logout user by resetting user session and redirecting to login page
 */
UnoController.logout = function(request, response){
  request.session = {};
  response.redirect('/');
}
